...
// 调用 compiler 生成 wxml
function genComponentWxml (compiled, options, emitFile, emitError, emitWarning, fileExt) {
  options.components['slots'] = { src: 'slots', name: 'slots' }
  const { code: wxmlCodeStr, compiled: cp, slots, importCode } = compiler.compileToWxml(compiled, options, fileExt)
...
function createAppWxml (emitFile, resourcePath, rootComponent, context, fileExt) {
  const { src } = getFileInfo(resourcePath) || {}
  const { name: componentName, filePath: wxmlSrc } = getCompNameAndSrc(context, rootComponent, fileExt)
  const fileName = `/${src}.${fileExt.template}`
  const wxmlContent = genPageML(componentName, relative(fileName, `/${wxmlSrc}`), fileExt)
  emitFile(fileName, wxmlContent)
}
// 更新全局组件时，需要重新生成wxml，用这个字段保存所有需要更新的页面及其参数
const cacheCreateWxmlFns = {}

function createWxml ({ emitWarning, emitError, emitFile, resourcePath, context, compiled, fileExt }) {
  ...
  //   moduleId: 'moduleId'
  // }
  const { name, filePath: wxmlSrc } = getCompNameAndSrc(context, resourcePath, fileExt)
  const options = { components, pageType, name, moduleId }
  const wxmlContent = genComponentWxml(compiled, options, emitFile, emitError, emitWarning, fileExt)
  emitFile(wxmlSrc, wxmlContent)
}

// 编译出 wxml
function compileWxml (compiled, html, options) {
...
function compileMPScript (script, mpOptioins, moduleId) {
  ...
  if (originComponents) {
    resolveSrc(originComponents, components, this.resolve, this.context, mpOptioins.fileExt).then(() => {
  ...
  if (isApp || isPage) {
    ...
    if (isPage && rootComponent) {
      resolve(context, rootComponent, (err, rootComponentSrc) => {
        if (err) return
        // 这儿需要搞定 根组件的 路径
        createAppWxml(emitFile, resourcePath, rootComponentSrc, this.options.context, mpOptioins.fileExt)
      })
    }
  }
...
function resolveSrc (originComponents, components, resolveFn, context, fileExt) {
  return Promise.all(Object.keys(originComponents).map(k => {
    return new Promise((resolve, reject) => {
      resolveFn(context, originComponents[k], (err, realSrc) => {
        ...
        const { filePath, name } = getCompNameAndSrc(context, realSrc, fileExt)
